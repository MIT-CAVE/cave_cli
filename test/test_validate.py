...
class TestValidateAppDir:
    ...
    def test_candidate_missing_folders_files(self, tmp_path):
        ...
        assert any(
            "Neither 'requirements.txt' nor 'pyproject.toml' exists." in err
            for err in res
        )

    def test_find_app_dir_reports_starting_dir_errors(
        ...
        any_missing_core = any(
            "The folder 'cave_core' is missing" in err for err in logged_errors
        )
